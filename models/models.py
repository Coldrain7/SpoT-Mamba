...
class SpoTMamba(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_steps: int = 12) -> torch.Tensor:
        ...
        # last dimension -> [traffic flow, timestamp of day, day of week]
    ...
    def  generate_walk_sequences(self, seed) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 将graph中的结点复制num_walks次
        nodes = graph.nodes().repeat_interleave(num_walks)

        # Generate Walk Sequences
        # p>1时不太可能是上一个顶点，<1时较为可能是上一个顶点，于是会在源结点的局部区域
        # q>1类似于BFS，<1类似于DFS
        # p,q值一般取2的幂
        # nodes是游走序列的起始节点，num_edges表示游走序列长度，即从起始结点开始游走的次数
        sequence_bfs, eids_bfs = dgl.sampling.node2vec_random_walk(graph, nodes, self.p_bfs, self.q_bfs, num_edges,
                                                                   return_eids=True)
        sequence_dfs, eids_dfs = dgl.sampling.node2vec_random_walk(graph, nodes, self.p_dfs, self.q_dfs, num_edges,
                                                                   return_eids=True)
        sequence_rw, eids_rw = dgl.sampling.node2vec_random_walk(graph, nodes, self.p_rw, self.q_rw, num_edges,
                                                                 return_eids=True)
