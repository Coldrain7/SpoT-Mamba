...
class Dataset_PEMS04(Dataset):
    ...
    def __read_data__(self):
        ...
        data_raw = np.load(os.path.join(self.root_path,
                                        self.data_path))["data"]
        data_flow = data_raw[:, :, 0]
        num_data = data_flow.shape[0] - (self.seq_len + self.label_len)
        train_end = int(num_data * 0.6 + 0.5)
        val_end = train_end + int(num_data * 0.2 + 0.5)
        test_end = num_data + 1
        # 开始边界
        ...
        if self.scale:
            train_data = data_flow[border1s[0]:border2s[0]]
            self.scaler.fit(train_data)
            data = self.scaler.transform(data_flow)
        else:
            data = data_flow
